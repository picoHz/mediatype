...
mod tests {
    ...
    fn parse() {
        assert_eq!(
            MediaTypeBuf::from_str("text/plain").unwrap().to_string(),
            "text/plain"
        );
        assert_eq!(
            MediaTypeBuf::from_str("text/plain;").unwrap().to_string(),
        ...
        assert_eq!(
            MediaTypeBuf::from_str("image/svg+xml").unwrap().to_string(),
            "image/svg+xml"
        );
        assert_eq!(
        ...
        assert_eq!(
            MediaTypeBuf::from_str("multipart/form-data ; boundary=--boundary13234")
                .unwrap()
                .to_string(),
            "multipart/form-data; boundary=--boundary13234"
        );

        let s = "text/plain";
        let long_str = format!("{};{}", s, " ".repeat(std::u16::MAX as usize - 2 - s.len()));
        assert_eq!(
            MediaTypeBuf::from_str(&long_str).unwrap().to_string(),
            "text/plain"
        );
    }
    ...
    #[test]
    fn parse_error() {
        assert_eq!(MediaTypeBuf::from_str(""), Err(ParseError::InvalidTypeName));
        assert_eq!(
            MediaTypeBuf::from_str("textplain"),
            Err(ParseError::InvalidTypeName)
        );
        assert_eq!(
            MediaTypeBuf::from_str("text//plain"),
        ...
        assert_eq!(
            MediaTypeBuf::from_str("text/plain; charsetutf-8"),
        ...
        assert_eq!(
            MediaTypeBuf::from_str("text/plain; charset=utf-8; charset=utf-8"),
            Err(ParseError::DuplicatedParamKeys)
        );
        assert_eq!(
            MediaTypeBuf::from_str("text/plain; charset=\"utf-8"),
        ...
        assert_eq!(
            MediaTypeBuf::from_str(&long_str),
